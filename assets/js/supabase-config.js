...
if(isAdminRuntime){
  ...
  if(!document.querySelector('link[data-azo-obras-style]')){
    ...
    style.href='./obras.css';
    style.dataset.azoObrasStyle='1';
    document.head.appendChild(style);
  }
  import('../../admin/obras.js').catch(error=>console.warn('[AZO Studio] Módulo Obras indisponível.',error));
}else{
  import('./project-overrides.js').catch(error=>console.warn('[AZO] Alterações de projetos existentes indisponíveis.',error));
  import('./obras-public.js').catch(error=>console.warn('[AZO] Navegação de obras indisponível.',error));
}
