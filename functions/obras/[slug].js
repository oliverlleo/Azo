...
function highlights(row) {
  const items = Array.isArray(row.highlights) ? row.highlights.filter(Boolean) : [];
  if (!items.length) return '';
  return `<div class="obra-highlights">${items.map((item, index) => `
    <article class="obra-highlight"><span>${String(index + 1).padStart(2, '0')}</span><strong>${escapeHtml(item)}</strong></article>`).join('')}
  </div>`;
}
...
function story(row) {
  ...
  let number = 0;
  const index = [];
  if (hasIntro) index.push(`<a href="#obra"><span>${escapeHtml(row.intro_title || 'A obra')}</span><b>${String(++number).padStart(2, '0')}</b></a>`);
  if (hasChallenge) index.push(`<a href="#desafio"><span>${escapeHtml(row.challenge_title || 'O desafio')}</span><b>${String(++number).padStart(2, '0')}</b></a>`);
  if (hasSolution) index.push(`<a href="#solucao"><span>${escapeHtml(row.solution_title || 'A solução')}</span><b>${String(++number).padStart(2, '0')}</b></a>`);
  if (hasHighlights) index.push(`<a href="#destaques"><span>Decisões</span><b>${String(++number).padStart(2, '0')}</b></a>`);
  ...
  return `<section class="obra-story"><div class="wrap obra-story__grid">
    ...
    <div class="obra-story__body">
      ${hasIntro ? `<article class="obra-story-block obra-reveal" id="obra"><h2>${escapeHtml(row.intro_title || 'A obra')}</h2><p>${escapeHtml(row.intro_body)}</p></article>` : ''}
      ${hasChallenge ? `<article class="obra-story-block obra-reveal" id="desafio"><h2>${escapeHtml(row.challenge_title || 'O desafio')}</h2><p>${escapeHtml(row.challenge_body)}</p></article>` : ''}
      ${hasSolution ? `<article class="obra-story-block obra-reveal" id="solucao"><h2>${escapeHtml(row.solution_title || 'A solução')}</h2><p>${escapeHtml(row.solution_body)}</p></article>` : ''}
      ${hasHighlights ? `<article class="obra-story-block obra-reveal" id="destaques"><h2>Decisões que<br>fazem diferença.</h2>${highlights(row)}</article>` : ''}
    </div>
  </div></section>`;
}
...
function servicesSection(row) {
  ...
  return `<section class="obra-services"><div class="wrap obra-services__grid">
    <div class="obra-reveal"><div class="eyebrow">Atuação AZO</div><h2>Decisões<br>conectadas.</h2></div>
...
function render(row, allRows) {
  ...
  return `<!doctype html><html lang="pt-BR"><head>
    ...
    <link rel="stylesheet" href="/assets/css/site.css"><link rel="stylesheet" href="/assets/css/obra-page.css"><script type="application/ld+json">${safeJson(schema)}</script>
    ...
    <main>
      <section class="obra-hero"><div class="obra-hero__media">${heroMedia(row)}</div><div class="obra-hero__veil" style="opacity:${Math.max(.5, overlay / 60)}"></div><div class="obra-hero__blueprint"><svg viewBox="0 0 1600 900" preserveAspectRatio="none" aria-hidden="true"><path d="M70 170H510V65H980V245H1530"/><path d="M170 805V515H615V335H1210V745H1515"/><rect x="280" y="190" width="620" height="420"/><path d="M900 190v420M280 400h620M540 190v210M690 400v210"/></svg></div><div class="obra-hero__orb"></div><div class="wrap obra-hero__content"><div class="obra-hero__eyebrow">${escapeHtml(row.eyebrow || 'Obra AZO')}</div><h1>${escapeHtml(row.title)}</h1><div class="obra-hero__bottom"><p class="obra-hero__intro">${escapeHtml(row.excerpt || '')}</p>${facts(row)}</div></div><a class="obra-scroll" href="#conteudo" aria-label="Conhecer a obra">↓</a></section>
      <div class="wrap obra-breadcrumb"><a href="/">Início</a> / <a href="/obras/">Obras</a> / ${escapeHtml(row.title)}</div><div id="conteudo"></div>
    ...
    <script src="/assets/js/obra-page.js" defer></script>
  </body></html>`;
}
