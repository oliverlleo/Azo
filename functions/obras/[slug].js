...
function heroMedia(row) {
  ...
  if (row.hero_media_type === 'video' && video) {
    return `<video src="${escapeHtml(video)}" autoplay muted loop playsinline preload="auto" ${poster ? `poster="${escapeHtml(poster)}"` : ''} style="object-position:${x}% ${y}%"></video>`;
  }
...
function story(row) {
  ...
  const tabs = sections.map((section, index) => `<button type="button" id="story-tab-${section.id}" role="tab" data-story-target="${section.id}" aria-controls="story-panel-${section.id}" aria-selected="${index === 0 ? 'true' : 'false'}" tabindex="${index === 0 ? '0' : '-1'}"><span>${String(index + 1).padStart(2, '0')}</span><strong>${escapeHtml(section.title)}</strong><i aria-hidden="true"></i></button>`).join('');
  const panels = sections.map((section, index) => `<article id="story-panel-${section.id}" class="obra-story-block obra-reveal" role="tabpanel" aria-labelledby="story-tab-${section.id}" tabindex="0" data-story-panel="${section.id}" ${index === 0 ? '' : 'hidden'}><div class="obra-story-block__kicker">${String(index + 1).padStart(2, '0')} · ${escapeHtml(section.kicker)}</div><h2>${escapeHtml(section.title)}</h2>${section.body ? `<p>${escapeHtml(section.body)}</p>` : ''}${section.highlights ? `<div class="obra-highlights">${section.highlights.map((item, itemIndex) => `<article class="obra-highlight"><span>${String(itemIndex + 1).padStart(2, '0')}</span><strong>${escapeHtml(item)}</strong></article>`).join('')}</div>` : ''}</article>`).join('');
...
function gallerySection(row) {
  ...
  return `<section class="obra-gallery" id="galeria"><div class="wrap">
    ...
    <div class="obra-gallery__grid">${items.map((item, index) => `
      <button type="button" class="obra-gallery__item obra-reveal" aria-label="Abrir imagem: ${escapeHtml(item.alt || `${row.title} — imagem ${index + 1}`)}"><img src="${escapeHtml(absoluteUrl(item.url))}" alt="${escapeHtml(item.alt || `${row.title} — imagem ${index + 1}`)}" loading="${index < 2 ? 'eager' : 'lazy'}" decoding="async">${item.caption ? `<span class="obra-gallery__caption">${escapeHtml(item.caption)}</span>` : ''}</button>`).join('')}
    </div>
  </div></section>`;
}
...
function render(row, allRows) {
  ...
  return `<!doctype html><html lang="pt-BR"><head>
    ...
    <link rel="stylesheet" href="/assets/css/site.css?v=20260821-1265-a11y"><link rel="stylesheet" href="/assets/css/obra-page.css?v=20260821-1265-a11y"><script type="application/ld+json">${safeJson(schema)}</script>
  </head><body class="obra-page">
    <header class="site-header obra-site-header"><a class="brand" href="/" aria-label="AZO — início"><img src="/assets/images/logo-azo.png" alt="AZO Criação & Construção" width="760" height="213"></a><nav class="nav" aria-label="Navegação desta obra">${sectionLinks}<a class="nav-cta" href="/contato.html">Solicite um contato</a></nav><button class="menu-toggle" aria-label="Abrir menu" aria-expanded="false" aria-controls="mobile-navigation"><span></span></button></header>
    <nav id="mobile-navigation" class="mobile-nav obra-mobile-nav" aria-label="Navegação desta obra">${sectionLinks}<a href="/contato.html">Solicite um contato</a><small>${escapeHtml(row.title || 'Obra AZO')}</small></nav>
    <main>
    ...
    <div class="obra-lightbox" role="dialog" aria-modal="true" aria-labelledby="obra-lightbox-title" aria-hidden="true" hidden><div class="obra-lightbox__top"><div><div id="obra-lightbox-title" class="obra-lightbox__title"></div><div class="obra-lightbox__count"></div></div><button class="obra-lightbox__close" aria-label="Fechar">×</button></div><div class="obra-lightbox__stage"><img alt=""></div><div class="obra-lightbox__controls"><button class="obra-lightbox__prev" aria-label="Imagem anterior">← Anterior</button><button class="obra-lightbox__next" aria-label="Próxima imagem">Próxima →</button></div></div>
    <script src="/assets/js/obra-page.js?v=20260821-1265-a11y" defer></script>
  </body></html>`;
}
