...
<meta name="description" content="${attr(description)}"><link rel="canonical" href="${attr(canonical)}">
...
})}</script>${articleJson?`<script type="application/ld+json">${articleJson}</script>`:''}
...
function card(post,index=0){
  ...
  return `<a class="content-card content-reveal" data-delay="${Math.min(index+1,3)}" href="/conteudos/${encodeURIComponent(post.slug)}/">
    <div class="content-card__media"><img src="${attr(cover)}" alt="${attr(post.cover_alt||post.title)}" loading="lazy"></div>
    <div class="content-card__body"><div class="content-card__meta"><span>${esc(category)}</span><span>${esc(dateFmt(date))}</span></div><h3>${esc(post.title)}</h3><p>${esc(post.excerpt||'Leia o conteúdo completo da AZO.')}</p><span class="content-read">Ler conteúdo <i>→</i></span></div>
  </a>`;
}
...
async function listing(categorySlug=''){
  ...
  let category=null;
  if(categorySlug) category=categories.find(c=>c.slug===categorySlug)||null;
...
function renderList(data){
  ...
  return `<ul>${items.map(i=>`<li>${esc(typeof i==='string'?i:(i?.text||''))}</li>`).join('')}</ul>`;
}
function renderTable(data){
  const headers=Array.isArray(data?.headers)?data.headers:[]; const rows=Array.isArray(data?.rows)?data.rows:[];
  return `<table class="article-table">${headers.length?`<thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead>`:''}<tbody>${rows.map(r=>`<tr>${(Array.isArray(r)?r:[]).map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
}
...
async function article(slug){
  const posts=await rest('content_posts',{select:'id,title,slug,excerpt,cover_url,cover_alt,seo_title,seo_description,canonical_url,social_image_url,author_name,published_at,scheduled_at,updated_at,related_project_keys,related_service_keys,content_categories(name,slug)',slug:`eq.${slug}`,limit:'1'});
  const post=posts[0];
  if(!post) return null;
  ...
  const mins=readingMinutes(post,blocks);
  const canonical=post.canonical_url||`${SITE}/conteudos/${post.slug}/`;
  const title=post.seo_title||post.title;
  const description=post.seo_description||post.excerpt||`Conteúdo da AZO sobre ${post.title}.`;
...
async function redirectFor(path){
  try{
    const rows=await rest('content_redirects',{select:'source_path,destination_path,status_code',source_path:`eq.${path}`,limit:'1'});
    return rows[0]||null;
  }catch(_){return null;}
}
...
export async function onRequestGet(context){
  try{
    ...
    const parts=(Array.isArray(raw)?raw:String(raw||'').split('/')).filter(Boolean).map(decodeURIComponent);
    if(!parts.length){
      return new Response(await listing(),{headers:{'content-type':'text/html; charset=utf-8','cache-control':'public,max-age=60,s-maxage=300'}});
    }
    if(parts[0]==='categoria'&&parts[1]){
      return new Response(await listing(parts[1]),{headers:{'content-type':'text/html; charset=utf-8','cache-control':'public,max-age=60,s-maxage=300'}});
    }
    const slug=parts[0];
    const html=await article(slug);
    if(html) return new Response(html,{headers:{'content-type':'text/html; charset=utf-8','cache-control':'public,max-age=60,s-maxage=300'}});
    const path=`/conteudos/${parts.join('/')}/`;
    const redirect=await redirectFor(path);
    if(redirect) return Response.redirect(new URL(redirect.destination_path,SITE),redirect.status_code||301);
    return new Response(shell({title:'Conteúdo não encontrado | AZO',description:'O conteúdo solicitado não está disponível.',canonical:`${SITE}${path}`,body:'<section class="content-shell"><div class="wrap content-section"><div class="content-empty"><strong>Conteúdo não encontrado.</strong><a href="/conteudos/">Voltar para Conteúdos →</a></div></div></section>'}),{status:404,headers:{'content-type':'text/html; charset=utf-8'}});
  }catch(error){
    return new Response(`<!doctype html><meta charset="utf-8"><title>Conteúdos AZO</title><h1>Conteúdos temporariamente indisponíveis</h1><p>${esc(error.message)}</p>`,{status:503,headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store'}});
  }
}
